...
The centroid positions $\mu$ of the $^{40}$Ca states along the P1 focal-plane spectra were used to calibrate their excitation energies, given several known calibration states. These $\mu$ were obtained with high precision through the Bayesian peak fitting procedure, detailed in Section \ref{sec:peak_fitting}. However, the centroid uncertainty obtained from the fit does not represent the true channel uncertainty. The fit underestimates the uncertainty, as it does not take into account scatter associated with the focal-plane detector response. An additional scatter uncertainty of $u_{\mathrm{s}} \sim 0.6$ channels was added to rectify this, corresponding to roughly 1 keV in excitation energy. The new channel uncertainty associated with each centroid value is
\begin{equation}
...
The energy calibration was performed using a Markov chain Monte Carlo (MCMC) fit with over 50,000 iterations and for 25,000 samples.


% 2.) MCMC Residual figure for 5 deg:
% Uncertainty bars shown for the test states are simply the channel uncertainty converted to energy from the fit $u_{\mathrm{ch-E}}$ added in quadrature with the ENSDF uncertainties. The final uncertainties reported for each sort group are $u_{\mathrm{ch-E}}$ added in quadrature with half the $1\sigma$ width from the fit.
...
% 6.) The energy-calibrated position spectrum, focused on the astrophysical region.
...
\centering
\caption{\label{tab:partialwidth}Excitation energies $E_{x}$ determined through a Markov Chain Monte Carlo (MCMC) energy calibration for the $^{40}$Ca states observed in the present experiment. For comparison, the ENSDF \cite{Chen2017} excitation energies and $J^{\pi}$ values are also given, unless otherwise indicated. Brackets represent states from ENSDF too close in energy to resolve via the energy calibration alone. Calibration states are shown in italics.}
...
\caption{\label{fig:residuals_5deg}Residuals between the excitation energies predicted from the MCMC energy calibration and that of ENSDF \cite{Chen2017} at $\theta_{\mathrm{lab}} = 5^{\circ}$ (sort group $\#$3) for the calibration states (in red) and the test states (in black). The $68\%$ ($1\sigma$) and $95\%$ ($2\sigma$) confidence intervals for the fit are represented by the dark and light shading, respectively. See text for details.}
\end{figure}
