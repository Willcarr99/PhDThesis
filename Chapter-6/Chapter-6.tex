...
%\subsection{Focal Plane Detector?}
...
\begin{figure}[t]
\centering
\includegraphics[width=6.5in]{Chapter-6/figs/5deg_Runs83_84_85_86_87_withC.png}
\caption{\label{fig:5deg_fp}A histogram spectrum of the front (P1) focal-plane detector position section at $\theta_{\mathrm{lab}} = 5^{\circ}$ with the KI $\#$6 target (in red) and the carbon target (in black), gated on $d$ from the the DE vs. E spectrum. The $x$-axis shows the magnetic rigidity $B\rho$, and the $y$-axis shows the number of $d$ counts.}
\end{figure}
...
\end{tikzpicture}
\caption{\label{fig:9deg_P1vsEvt}A 2D histogram of the front (P1) focal-plane position section vs event number over all runs at $\theta_{\mathrm{lab}} = 9^{\circ}$ with the KI $\#$5 target. Both axes are compressed to 512 bins. The P1 section is zoomed in on the $^{12}$C gound state to show the effect of energy shifts due to beam retunes. A beam retune happened after run $\#$25, causing the shift in the P1 spectrum shown at about event (bin) number 320.}
\end{figure}
