...
\centering
%\caption{\label{tab:spec}Spectroscopic factors of the present experiment, compared with that of the other $(^{3}\mathrm{He},d)$ \cite{Erskine1966,Seth1967,Forster1970,Cage1971} and $(d,n)$ \cite{Fuchs1969} measurements in the literature.}
\caption{\label{tab:spec}(continued)}
\resizebox{\linewidth}{!}{%
%\hspace{0.5cm}
% - 0.25cm
%\hrule width \hsize \kern 1mm \hrule width \hsize height 0.4pt
...
% Longland et al. 2018 --- Why only narrow resonances contribute here:

% The 39 K(p, γ )40Ca reaction proceeds through the compound nucleus 40Ca at a high excitation energy (Sp = 8328.437(21) keV [12]). The average level density of 40Ca at these excitation energies is about 60 MeV−1 , corresponding to an average level spacing of about 20 keV. Of these levels, fewer will exhibit an appreciable proton width and contribute significantly to the reaction rate, as will become apparent in Sec. IV. 

%At the low proton energies relevant for astrophysics, the high Coulomb barrier renders the proton width in Eq. (4) to be small. Thus, the cross section can be considered to be dominated by narrow resonances and the reaction rate is calculated using Eq. (2). Interference effects are expected to average to a negligible contribution, and the nonresonant part of the cross section can be neglected.

The $^{39}\mathrm{K}(p,\gamma)^{40}\mathrm{Ca}$ reaction rate is dominated by narrow resonances 

As described in Section \ref{subsec:narrow_resonances}, narrow resonances contribute individually to thermonuclear reaction rates. The resonance strength $\omega\gamma$ and resonance energy $E_{r}$ of each resonance are the key experimental quantities that go into the reaction rate calculation of Eqn. \ref{eqn:narrow_rate}. The resonance strength
\begin{equation} \label{eqn:resonance_strength}
\omega\gamma = \omega \frac{\Gamma_{a}\Gamma_{b}}{\Gamma}
\end{equation}
depends on the partial widths of the incoming ($\Gamma_{a}$) and outgoing ($\Gamma_{b}$) channels and the total partial width $\Gamma$. The spin factor $\omega$ is defined as
\begin{equation}
\omega = \frac{(2J+1)}{(2J_{0}+1) (2J_{1}+1)},
\end{equation}
where $J$ is the resonance spin, $J_{0}$ is the target spin, and $J_{1}$ is the incoming particle spin. For $(p,\gamma)$ reactions, the incoming proton partial width $\Gamma_{p}$ and the outgoing $\gamma$-ray partial width $\Gamma_{\gamma}$ are the quantities of interest. If $\Gamma_{\gamma} >> \Gamma_{p}$, then
\begin{align*}
\omega\gamma &\approx \omega \frac{\Gamma_{p} \Gamma_{\gamma}}{\Gamma_{\gamma}} \\
&\approx \omega \Gamma_{p},
\end{align*}
which means the proton partial width dominates the resonance strength. This occurs for $^{39}\mathrm{K}(p,\gamma)^{40}\mathrm{Ca}$ at energies below roughly the $E_{r}^{\mathrm{c.m.}} = 606$ keV resonance, in the region of astrophysical interest.

The proton partial width can be written as
\begin{equation}
\Gamma_{p} = \frac{\hbar^{2}}{\mu R} \, C^{2}S \, P(E_{r}) \, \phi_{R}^{2},
\end{equation}
where $\mu$ is the reduced mass of the proton and target, $R$ is the channel radius given by $R =$

%\begin{table}
%\caption{\label{tab:partial_widths}}}
%\end{table}
