...
% TALK ABOUT THE GAMOW WINDOW (TEMPERATURES WHERE THE REACTION IS RELEVANT TO ASTROPHYSICS) - reference Ch 2 for discussion of the Gamow window. See my paper for temperature range and resonance (excitation) energy range. Then move on to Longland2018's reaction rate.

% Will's update to test github on Windows
% Will's update to test compiling with TexMaker in Windows, then pushing changes to github
% Will's update to test autocrlf config option in Windows, to get that annoying warning message to go away
