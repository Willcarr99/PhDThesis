...
\end{equation}
where $u_{\mathrm{sp}, \, c}(R)$ is the single-particle radial wavefunction evaluated at the channel radius, which is normalized as in
\begin{equation}
\int_{0}^{R} |u_{\mathrm{sp}, \, c}(R)|^{2} \, r^{2} \, dr = 1.
\end{equation}
One method of numerically calculating $\theta_{\mathrm{sp}, \, c}$ is to vary the parameters of the single-particle potential until a resonance is produced at the binding energy. This is performed, for example, by the code \texttt{BIND}, detailed in Ref. \cite{Iliadis1997}. Another procedure, performed for $^{39}\mathrm{K}(p, \gamma)^{40}\mathrm{Ca}$ in Sec. \ref{subsec:partial_widths}, is to use the weak binding approximation. This method assumes unbound states are weakly bound (by $\sim 1$ keV), and therefore $u_{\mathrm{sp}, \, c}$ is treated as the single-particle radial wavefunction for the bound state, which can be extracted from nuclear reaction codes, such as \texttt{Fresco} \cite{Thompson1988,Fresco}, and evaluated at the channel radius $R$. This approximation for narrow proton resonances has been shown to agree with proton partial widths calculated from the unbound approach to within 15$\%$. The agreement is in part due to the unbound proton wave functions being well described in the weak binding approximation because the high Coulomb barrier suppresses the wave function at large radii \cite{Hammache2021}.
