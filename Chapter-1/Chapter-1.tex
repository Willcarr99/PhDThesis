...
\node at (1+\BoxSpacehalf+0.25,1+\BoxSpacehalf) {\tiny{(a)}};
\node at (2+\BoxSpace+\BoxSpacehalf,0.65) {\tiny{(b)}};

\end{tikzpicture}
\vspace{0.75 cm}
\caption{\label{fig:sProcess_MathExample}A simple s-process path initiating from the nucleus with mass number $A-1$, where shaded nuclei are stable, and unshaded nuclei are unstable. Case (a) is shown by the solid arrows, where only $\beta$--decays occur on unstable nuclei. Case (b), indicated by the addition of dashed arrows, treats $A'$ as a branching point nucleus.}
\end{figure}
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
\section{Globular Cluster Abundance Anomalies} \label{sec:GC_Candidate}
